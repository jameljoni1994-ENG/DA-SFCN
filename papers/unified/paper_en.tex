...
The ARC Cauchy-point lemma yields a model decrease of order
$\|g_k\|^{3/2}/\sqrt{M_k}$ (or $\|g_k\|^2$ on the Newton branch).
Successful iterations therefore drive $\sum_i\|g_i\|^{3/2}<\infty$ after
accounting for the boundedness of $M_k$, which is the $K^{-2/3}$ bound.
Rejected iterations only inflate $M_k$ and are finite in number on any
window where $M_k$ would otherwise exceed $L_2$. Lemma~\ref{lem:sf}
ensures that replacing $T_k$ by $|T_k|$ cannot destroy the Cauchy decrease
in $\mathrm{span}\{g_k\}$.
\end{proof}
...
\begin{proof}[Proof sketch]
Write $e_k=x_k-x^\star$. Standard inexact-Newton calculus
\cite{dembo1982} gives
$\|e_{k+1}\|\le (L_2/(2\gamma))\|e_k\|^2+\rho^{(m_k)}\|e_k\|$.
Near $x^\star$ one has $\|g_k\|=\Theta(\|e_k\|)$, hence
$m_k\ge\alpha\log(1/\|e_k\|)+O(1)$ and
$\beta^{m_k}=O(\|e_k\|^{\alpha\log(1/\beta)})$.
The choice of $\alpha$ makes the perturbation $o(\|e_k\|^2)$.
\end{proof}
