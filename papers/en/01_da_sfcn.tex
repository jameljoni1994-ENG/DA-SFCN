...
\end{lemma}
\begin{proof}
If $T=U\Lambda U^\top$ and $z=U^\top s$ then
$|s^\top T s|=|\sum\lambda_i z_i^2|\le\sum|\lambda_i|z_i^2=s^\top|T|s$.
\end{proof}
...
\end{theorem}
\begin{proof}
Lanczos keeps $g_k$ in the Krylov space. The ARC Cauchy decrease on
$-\tau g_k$ yields $\Delta_k\gtrsim\|g_k\|^{3/2}/\sqrt{M_k}$. Successful
steps sum to $f(x_0)-f^\star\gtrsim\sum\|g_i\|^{3/2}$, hence the
$K^{-2/3}$ bound. Absolute spectrum preserves the lower model on
$\mathrm{span}\{g_k\}$. Full write-up: \texttt{pdfs/08\_Theory\_EN.pdf}.
\end{proof}
\begin{theorem}[Local]
...
Chebyshev gives $\rho^{(m)}\le 2\beta^m$; the log schedule forces
$\beta^{m_k}=O(\|e_k\|)$, absorbing the residual into the quadratic term.
\end{proof}
